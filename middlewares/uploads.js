...
const imageFilter = (req, file, cb) => {
    if (file.mimetype === 'image/jpg' || file.mimetype === 'image/jpeg' || file.mimetype === 'image/png' ||
        file.mimetype === 'image/gif' || file.mimetype === 'image/svg+xml' || file.mimetype === 'application/octet-stream') {
      cb(null, true);
    } 
    else {
      cb("Please upload only image file. (jpg, jpeg, png, gif, svg and svga)", false);
    }
};
...
const storage = multerS3({
    ...
    key: function (req, file, cb) {
        let originalFile = file.originalname, ext = '';
        console.log(file);
