    ...
    io.on('connection', (socket) => {
        ...
        socket.on('leaveRoom', async () => {
           // Retrieve userId and roomId from socket object
            ...
            if (!userId && !roomId) {
                logger.warn(`leaveRoom event triggered without userId and roomId for socket ${socket.id}`);
                console.log(`Warning: leaveRoom event triggered without userId and roomId for socket ${socket.id}`);
                return;
            }

            logger.info(`Received leaveRoom event with data: userId=${userId}, roomId=${roomId}`);
            console.log(`Received leaveRoom event with data: userId=${userId}, roomId=${roomId}`);
            ...
            try 
            {
                ...
                console.log(`Client ${socket.id} has been removed from room ${roomId} (${userId})`);
            } 
            catch (error) {
        ...
        socket.on('disconnect', async () => {
            ...
            try 
            {
                ...
                if(room.ownerId === userId)
                {
                    ...
                    {
                        const currentHostTime = moment().diff(moment(room.lastHostJoinedAt), 'seconds');
                        await models.Room.updateOne({ _id: roomId },
                            {
                                $set: {
        ...
        socket.on('sendGift', async (data) => {
            ...
            if(socket.data.userId && socket.data.roomId && sender && receiver && giftId)
            {
                try
                {
                    const roomId = socket.data.roomId;
                    const [customers, gift] = await Promise.all([
                        models.Customer.find({ _id: { $in: [sender, receiver] } }, 
                            {userId: 1, diamonds: 1, usedDiamonds: 1, xp: 1, beans: 1}).lean(),
                        models.Gift.findOne({ _id: giftId }).lean()
                    ]);

                    if (customers.length < 2 || !gift) {
                        logger.warn(`Invalid data: sender=${sender}, receiver=${receiver}, giftId=${giftId}`);
                        return;
                    }

                    const senderC = customers.find((customer) => customer._id.equals(sender));
                    const receiverC = customers.find((customer) => customer._id.equals(receiver));
                    ...
                    const resp = {
                        ...
                        diamondsUsedCurrentSeason: rooms.diamondsUsedCurrentSeason + giftDiamonds
                    };
                    ...
                    io.to(roomId).emit('giftUpdate', { receiver, giftId, count, sender, points: giftDiamonds * 3 });
